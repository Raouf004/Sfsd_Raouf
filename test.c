...
typedef struct {
    ...
    int first_block_addr; // Adresse du premier bloc
} MetaInfo;
...
void create_meta_info(File *file_obj, MetaInfo *meta_info) {
    ...
    meta_info->first_block_addr = (file_obj->blocks[0] != NULL) ? 
...
void insert_record(File *file_obj, const Record new_record) {
    if (count_records(file_obj) >= MAX_BLOCKS) {
